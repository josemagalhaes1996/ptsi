...
if(isset( $_SESSION['login'])){
 ...
 $id=$_REQUEST['id'];
$query="UPDATE `gardencorporation`.`compra` SET `estado` = 'Pagamento Efectuado' WHERE `compra`.`numero_compra` =$id;";
mysql_query($query);
...
$mensagem="A Empresa Garden Corporation vem por este meio informar o senhor/a "."$nome"."  que o respectivo pagamento da  compra numero ".$id." foi aprovada."."\r\n";
...
$query2="INSERT INTO `gardencorporation`.`auxiliar_compra` (`numero_compra`, `estado`, `login_func`, `data`) VALUES ('$id', 'Pagamento Aprovado', '$login', CURRENT_TIMESTAMP);";
...
$header = 'MIME- Version: 1.1'."/r/n";
 ...
 $envio = mail($email,'Aprovaçao de Pagamento',$mensagem,$header);
...
header("location:Aprova_Pagamento.php");
}else{
