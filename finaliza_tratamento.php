...
$id=$_REQUEST['id'];
$query="UPDATE `gardencorporation`.`compra` SET `estado` = 'Expedida' WHERE `compra`.`numero_compra` =$id;";
mysql_query($query);

$query2="INSERT INTO `gardencorporation`.`auxiliar_compra` (`numero_compra`, `estado`, `login_func`, `data`) VALUES ('$id', 'Compra Finalizada ', '$login', CURRENT_TIMESTAMP);";
...
$mensagem="A Empresa Garden Corporation vem por este meio informar o senhor/a "."$nome"."  que se Finalizou o tratamento da compra numero ".$id." , em breve ira receber a encomenda"."\r\n";
...
$header = 'MIME- Version: 1.1'."/r/n";
 ...
 $envio = mail($email,'Final da Compra Garden Corporation',$mensagem,$header);
